...
class Bovada(HandHistoryConverter):
    ...
    def readHandInfo(self, hand):
        ...
        if not hand.separateTablesByMaxSeats and not hand.maxseats:
            hand.maxseats = 9
    ...
    def readBlinds(self, hand):
        ...
        for a in self.re_PostSB.finditer(hand.handText):
            sbs.append(a)
        if sbs:
            if postsb!=sbs[0].groupdict():
                postsb = sbs[0].groupdict()
            ...
            self.allInBlind(hand, 'PREFLOP', sbs[0], 'small blind')
            del sbs[0]

        for a in self.re_PostBB.finditer(hand.handText):
            bbs.append(a)
        if bbs:
            if postbb!=bbs[0].groupdict():
                postbb = bbs[0].groupdict()
                ...
                self.allInBlind(hand, 'PREFLOP', bbs[0], 'big blind')
